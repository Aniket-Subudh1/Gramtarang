...
/**
 * Host routing. (In Next 16 this file convention is called `proxy`;
 * it was `middleware` before.)
 *
 * Treat this as an optimistic filter, NOT a security boundary. Next.js
 * has shipped repeated advisories where a crafted request skips this
 * layer entirely. The authoritative admin check lives in lib/session.ts
 * and runs inside every protected page and route handler.
 *
